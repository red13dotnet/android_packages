...
//                    }
     ...
     */
    private Uri uiSaveMessage(Message msg,Long messageId, Mailbox mailbox, Bundle extras) {
        final Context context = getContext();
        // Fill in the message
		...
		Log.d(TAG,"uiSaveMessage:uiAtts.size()="+uiAtts.size());

		atts_rk=Attachment.restoreAttachmentsWithMessageId(context,messageId);//cjq
		Log.d(TAG,"uiSaveMessage:atts_rk.length="+atts_rk.length);//cjq
		for(Attachment att:atts_rk)
		{
			...
			Log.d(TAG,"uiSaveMessage: (att != null) atts.add(attClone)->attClone="+attClone.mFileName);
            atts.add(attClone);//cjq
			}
        ...
        for (com.android.mail.providers.Attachment uiAtt: uiAtts) {
            ...
            if (attUri != null && attUri.getAuthority().equals(EmailContent.AUTHORITY)) {
				...
				Attachment att_new=convertUiAttachmentToAttachment(uiAtt, cachedFileUri, msg.mAccountKey);
				for(Attachment att_save:atts)
				{
					if(att_save.getContentUri().equals(att_new.getContentUri()))
					{
						should_add=false;
						Log.w(TAG,"uiSaveMessage:(att == null)but find same,not add.att_new.mFileName="+att_new.mFileName);
					}
				}
				if(should_add)
				{
                	// Convert external attachment to one of ours and add to the list
                	Log.i(TAG,"uiSaveMessage:(att == null)Convert  atts.add ="+att_new.mFileName);
                	atts.add(att_new);
				}
            }
        }
    ...
    private Uri uiSendDraftMessage(final long accountId, final Bundle extras) {
        ...
        final Uri messageUri = uiSaveMessage(msg, cur_messageId,mailbox, extras);
        // Kick observers
